...
class EncoderLayer(nn.Module):
    ...
    def forward(self, enc_input, enc_mask):
        ...
        pos_output = self.posffn_layer(out) # residual connect?
        enc_output = self.normal_layer2(pos_output+out)
...
class Encoder(nn.Module):
    ...
    def forward(self, enc_input):
        ...
        enc_mask = create_padding_mask(enc_input, enc_input, self.pad_idx)

        enc_attn_prob = []

        for layer in self.layers:
            enc_output, attn_prob = layer(enc_output, enc_mask)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, dec_input, enc_output, self_dec_mask, enc_dec_mask):
        input_q = dec_input
        input_k = dec_input
        input_v = dec_input
        residual_1 = dec_input
        attn_out, attn_prob = self.masked_attn(input_q, input_k, input_v, self_dec_mask)
        ...
        dec_output = self.normal_layer3(pos_output+enc_dec_output)
...
class Decoder(nn.Module):
    ...
    def forward(self, enc_input, dec_input, enc_output): # memory = seq of output of the last layer of encoder!
        ...
        enc_dec_mask = create_padding_mask(dec_input, enc_input, self.pad_idx) # ERROR POINT
        ...
        for layer in self.layers:
            dec_output, self_attn, enc_dec_attn = layer(dec_output, enc_output, self_mask, enc_dec_mask)
